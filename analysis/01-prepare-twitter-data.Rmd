...
We then join this to the census tract for KY. This takes a bit of time so we save the result to .rds to use later.
...
The first step is filtering out users that do not meet the filter criteria. 
...
For the remaining users, we 'score' locations and select the one most likely to be the home location. We will save the result to .rds again so we can use it in our subsequent analysis.
